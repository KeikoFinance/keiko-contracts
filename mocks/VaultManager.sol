...
contract VaultManager is IVaultManager, AddressBook {
    ...
    */
    ...
    // REMOVE IN PROD
    function checkVault(address coll, address owner) public view returns( uint256, uint256, uint256) {
        Vault memory userVault = userVaults[owner][coll];

        return (userVault.collateral, userVault.debt, userVault.mcr);
    }

    // TEST FUNCTION REMOVE IN PROD
    ...
    */
    ...
    // USE PRICEFEED IN PROD
    function calculateCR(address vaultCollateral, address vaultOwner) public view returns (uint256) {
        Vault memory userVault = userVaults[vaultOwner][vaultCollateral];

        if (userVault.debt != 0) {
            uint256 price = collPrice; // uint256 price = IPriceFeed(priceFeed).fetchPrice(vaultCollateral);
    ...
    */
    ...
    function setMinRange(address collateral, uint256 minRange) public onlyOwner {
        ...
        emit MCRUpdated(collateral, minRange);
    }
