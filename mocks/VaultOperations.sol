...
contract VaultOperations is AddressBook {
    ...
    uint256 public collPrice = 6 * 1e18; // DELETE IN PROD
    ...
    event VaultRedeemed(address indexed vaultCollateral, uint256 amount);
    ...
    */
    function liquidateVault(address vaultCollateral, address vaultOwner, address prevId, address nextId) external {
        ...
        if (debtToOffset == debtAmount) {
            console.log("FULL LIQ");
            console.log("COLLATERAL FOR SP: ");
            console.log(spDistribution);
            ...
            console.log("REMAINING COLL FOR USER: ");
            console.log(remainingCollateral);
            if (remainingCollateral > 0) {
                IERC20(vaultCollateral).transfer(vaultOwner, remainingCollateral);
                console.log("Sent remaining collateral to user: ", remainingCollateral);
            }
    ...
    function redeemVault(address vaultCollateral, uint256 redemptionAmount, address prevId, address nextId) external {
        ...
        while (redemptionAmount > 0 && currentVault != address(0)) {
            ...
            unchecked {
                ...
                uint256 collateralForRedeemer = (netRedeemableAmount * 1e18) / collPrice;
        ...
        emit VaultRedeemed(vaultCollateral, redemptionAmount);
    }
    ...
    * @param vaultCollateral Address of the ERC20 token used as collateral.
    ...
    */
    ...
    /*
    * @notice Calculates the accrued interest on a given debt over a specified period of time.
    ...
    * @param _lastUpdated The timestamp at which the debt was last updated.
    * @param _currentTime The current timestamp to calculate the accrued interest up to.
    * @return The amount of interest accrued over the time period.
    */
    function calculateAccruedInterest(uint256 _currentDebt, uint256 _interestRate, uint256 _timeElapsed) internal pure returns (uint256) {
        ...
        // Convert annual interest rate to per-minute rate
        // _interestRate is already in percentage form with 18 decimals (e.g., 2.5% = 25 * 1e15)
        uint256 baseRate = DECIMAL_PRECISION + (_interestRate / MINUTES_IN_YEAR);
    ...
    */ // CHANGE TO INTERNAL AFTER FINISHING TESTS
    function calculateLiquidationDistribution(address _collateral, uint256 _collateralAmount, uint256 _debt) public view returns (uint256, uint256) {
        uint256 collateralPrice = collPrice; // Assuming collPrice is in 1e18 format. //IPriceFeed(priceFeed).fetchPrice(_collateral);
        ...
        if (liquidationAmount > collateralValue) {
            console.log("Return full collateral amount if liquidationAmount exceeds collateral value");
    ...
    // TEST FUNCTIONS - DELETE IN PROD
    function collateralBalanceOf(address vaultCollateral, address vaultOwner) public returns (uint256) {
        (uint256 currentCollateralAmount,,) = manageDebtInterest(vaultCollateral, vaultOwner);
